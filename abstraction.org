...
#+begin_src python :noweb yes
  def match(partial_data, prior_data):
      if is_variable(partial_data):
          <<return a positive match binding the variable to the prior_data>>
          ...
          #add case for when prior_data is empty list?
...
#+name: variable_case_list_case_for_match
#+begin_src python :noweb yes
  if element == partial_data[-1]:
      ...
      try:
          index_in_prior = prior_data.index(next_element)
          <match prefixes up to index_in_prior>
      except ValueError:
          return None
