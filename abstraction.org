...
#+begin_src python :noweb yes :tangle yes
  def match(partial_data, prior_data):
      if is_variable(partial_data):
          ...
          if isinstance(prior_data, list) and len(prior_data) >= partial_data:
...
#+begin_src python :noweb yes :tangle yes
  ...
  for partial_data_position, partial_data_element in enumerate(partial_data)
      if is_variable(element):
          ...
          matches.append(match(element, prior_data[prior_data_position]))
...
#+begin_src python :noweb yes :tangle yes
  if element == partial_data[-1]:
      return {element : prior_data[prior_data_position:]}
  else:
...
#+begin_src python :noweb yes :tangle yes
  ...
  last_position_to_check = len(prior_data[prior_data_position:]) - len(partial_data[partial_data_position]) + 1 + prior_data_position
  for i,prior_element in prior_data[prior_data_position:last_position_to_check]:
...
Combining the result of a match is basically merging two environments.  Since the environments are dictionaries we can just combine the entries of the two.
