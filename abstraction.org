...
This section will describe more formally the simple algorithm for trying to solve inference problems introduced in the section "First Approximation."  Recall the idea is to create a solution, i.e. binding of variables, based on matching the partial data of the inference problem to previously seen data.
...
What should be the output of a matching e.g. if I try to match [[1, 1, 1, 1], 'v0'] with [[1], 0] what should the result be?  What about [[1, 1, 1, 1], 'v0'] and [[1, 1, 1, 1, 1], 0]?

If there is an obvious match e.g. in the case of [[1, 1], 'v0'] and [[1, 1], 0] we would like to return at least the binding {'v0': 0}.  Another possible thing to return is a score for the match.  Are there any other things we might want to return?  What are the consequences of using the different return types?

In all cases it seems natural that the return of a matching should be a function of matching the individual elements.

******* Returning Only a Binding
******* Returning a Binding with a Score
